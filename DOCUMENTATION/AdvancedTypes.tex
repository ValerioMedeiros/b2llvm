...
representation LLVM starts at 0.
...
%TODO: insert a rule to do the shift address with the first element of interval
\begin{align*}
...
%TODO: support to shift between interval
\begin{itemize}
	\item $array2 \in (10..50) \times ((-10)..100) \to INT $   (a multidimensional array)
\end{itemize}

The array indexes can composed of only primitive types (INTEGER and BOOLEAN). To access an element the formula is $address(e(i1,..,in))  = i1-i1\_start, ... , in-in\_start$.

%TODO create a new rule based on the below rule.


 
%it must change the declaration and the rule of ArrayItem
