...
\begin{tabular}[t]{rcl}   
  	\hline
        ...
        \B{rhs} & : & \B{Expr} \\
	\hline
	\multicolumn{3}{|c|}{Label value: \B{LValue}} \\	
	\hline
	...
	\hline
        ...
        \hline
        \multicolumn{3}{c}{\B{Lit}\alt\B{Term}\alt\B{Pred}\alt\B{Cons}\alt\B{Record}}\\
	\multicolumn{3}{c}{\B{Vari}\alt\B{StructItem}\alt\B{ArrayItem} } \\
        \hline
...
\begin{tabular}[t]{rcl}
	...
	\multicolumn{3}{c}{\B{Struct} \alt \B{ArrayType}}\\
	\hline
	...
	\B{type} & : & \B{Type}\\
	\hline
	\multicolumn{3}{|c|}{Record Structure : \B{Record}} \\
        \hline
	\B{elements} & : & seq \B{Name}\\
	\B{values} & : & seq \B{Expr}\\
	\hline
...
\VGM{To add in text: The nodes \B{Int\_lit} and \B{Bool\_lit} are literals in its 
respective set.}
\VGM{The LLVM grammar with support to array multidimensional.}
...
& \quad \IF \mbox{\B{n} is an array item} \THEN  \trad{n}{ArrayItem} \\
& \quad \IF \mbox{\B{n} is a struct item} \THEN  \trad{n}{StructItem} \\
...
Two approaches are possible to support arrays and structures. An approach is importing reusable libraries of AtelierB that are reported in the official document of ClearSy\footnote{\url{http://www.atelierb.eu/ressources/DOC/english/reusable-components-reference.pdf}}. This approach is supported, but the generated code is more complex and it needs more intermediate steps to manipulate the data of imported components. Moreover, this approach allows dynamic allocation for arrays, but it is strongly not recommended to safety-critical systems. A second approach enjoys the direct correspondence between B0 native arrays and LLVM-IR and this approach is presented here.

\subsection{Structure}
The struct declaration defines a list of variables to be placed under one name, 
allowing several variables to be accessed by one base name. This example get andset one value of xx'balance and initializes a structure. 
...
The following rule get an element value from the struct. The same rule is used in left side of attribution as a $LValue$ and right side of attribution as an $Expression$. The functions \SBPos(n) and \SBType(n)  get respectively the position and type of $n$ element in the structure. 
...
The attribution from record structure to a structure variable, line 1 from last code example, is translated by a sequence of application of rule $StructItem$.
...
sets. The node Array in AST represents a declaration of array or a 
...
The following example initialises a new array in C:

\begin{ccode}
int32_t arr[1][2][1]= {{{4},{5}}};
\end{ccode}

The syntax rule used to initialize an array in LLVM is:\\
$@<var\_name> = constant [<size> x <type>] [<type> <value>]$.\\
...
store i32 7, i32* %7, align 4;setting an element
\end{llvmcode}
