...
\end{tabular} 
  ...
  \begin{tabular}[t]{rcl}
	...
	\B{type} & : & seq \B{Type}\\
	\hline
%  	\multicolumn{3}{|c|}{Data record : \B{Record}} \\
%        \hline
%	\B{id} & : &  \B{Name}\\
%        \B{elements} & : & seq \B{Name} \\
%	\B{type} & : & \B{Struct}\\
%	\hline
	...
	\B{ran} & : & \B{Set}\\
        ...
        \hline
	\multicolumn{3}{c}{ seq \B{Map} \alt \B{Product} }\\
	\hline
...
following code declares an array global called ``Array3'' with 6000 (10*20*30)
integer numbers with 32 bits and initialized with zero.
...
%TODO: Explain the other rules of Array 
%- Declaring the type of array  a : (INT X INT) --> INT
%- Declaring a array dynamically {1|->2|->3, 2|->3|->4}
...
\begin{llvmcode}
  %7 = getelementptr [10 x [20 x [30 x i32]]]* @Array3, i32 0, i64 2, i64 3, i64 4
\end{llvmcode}
...
selects the first element in the pointer (* @Array3). The last elements 
(i64 2, i64 3, i64 4) selects the value from position (2,3,4) storing it in \%7.
This array is multidimensional then several index must be declared separated
...
%6 = sext i32 %5 to i64
%7 = getelementptr[10x[20x[30xi32]]]*@Array3,i32 0,i64 %6,i64 %4,i64 %2;getting an element
...
% Rule to get a element in array, it can be used in become equal or Expressions
\begin{align*}
...
&  \LET r \sep p_2  = \trad{n.indices}{LRExp}  \IN \\
...
%TODO: check this rule with David 

\begin{align*}
&\trad{n}{LRExp} \isdef \LET p \sep v \sep t  = \trad{n}{Exp}  \IN  \llvm{",\PH{t} \PH{v} "} \sep p\\
...
\begin{minted}[frame=single,bgcolor=bg,linenos=true,fontsize=\scriptsize]{c}
MACHINE Rec
SETS
 ID={aa,bb}
...
\begin{minted}[frame=single,bgcolor=bg,linenos=true,fontsize=\scriptsize]{llvm}
