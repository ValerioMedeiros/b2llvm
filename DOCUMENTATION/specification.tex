...
\newminted{pascal}{frame=single,firstnumber=1,linenos=true,fontsize=\scriptsize,bgcolor=white}
...
\begin{tabular}[t]{rcl}
	...
	\hline
	\multicolumn{3}{c}{ \B{start}, \B{end} : \B{Exp} } \\
	\hline
...
\begin{llvmcode}
%struct.T_1 = type { i32, i32 }
%struct.T_2 = type { i32, %struct.T_1 }
%struct.state = type { %struct.T_1, %struct.T_2 }

@name2 =  global %struct.T_2 zeroinitializer
@name1 =  global %struct.T_1 zeroinitializer
\end{llvmcode}

%\AND l \BE \local(\B{Structure.elements})  \IN  \llvm{"\PH{l}= type \{\PH{tps}\}\nl"}
...
\[
\textcolor{gray}{arr \in} \underbrace{( \underbrace{(0..1) \times (0..1) \times (0..3)}_{seq\ Set} \to \underbrace{0..1000}_{Set})}_{ArrayType}
\]
\end{description}


Declaring an array: the essential elements of the array are its name, size and type. 
The following code declares a multidimensional global array called ``Arr'' 
with 16 (2*2*4) integer numbers with 32 bits.

\begin{llvmcode}
%struct.state = type { ... , [2 x [2 x [4 x i32]]],.. }
\end{llvmcode}

The function ArrayType returns the type for a declared variable.

\begin{align*}
\trad{Array}{\B{Type}} \isdef \\
& \trad{Array.dom , Array.ran.type}{ArrayType}\\
\trad{seqSet,type}{\B{ArrayType}} \isdef \\
& \LET h \BE \B{head}(seqSet) \AND\\
& times \BE h.end - h.start + 1 \IN\\
& \IF seqSet.length == 1 \THEN\\
& \llvm{"[\PH{times} x \PH{type}] \nl"} \\
& \ELSE \\
& \LET res \BE \trad{tail(seqSet),type}{ArrayType} \IN \\
& \llvm{"[\PH{times} x \PH{res}] \nl"} \\
\end{align*}

%TODO
\VGM{[[ TODO: Write about the initialization (Structure and Array) unsing the concept of module/instance]]}

This current specification of the code generator supports also multi dimensional array.
These data types and logic/math operators compose expressions that are explained
in the next section.
...
%[No futuro colocar a regra aqui ou apenas descrever como ela seria implementada]
