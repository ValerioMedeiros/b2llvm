...
the state of each component of the module, which we name \llvm{\%argN}, where
\llvm{N} is a numeric identifier. The body of this function is composed as
follows: first frame stack memory is allocated, to store the representation of
the local variables in the initialization; second, all imported modules are
bound to the component's elements and initialized; last the instructions
composing the initialization clause are executed:

\begin{align*}
  \trad{\B{n}}{\B{init}}_{\text{interface}} \isdef
  & \llvm{"define void \PH{\init(\B{n})} ( \PH{\stateref(\B{n})} \%self,
    \PH{\trad{\trimportedinstances(\B{n})}{\B{args}}} \{ \nl"} \\
  & \quad\llvm{entry: \nl} \\
  & \quad\llvm{\PH{\trad{i.init}{alloc}}} \\
...
Next, we specify how to generate the code to enumerate the list of arguments
representing the components. Essentially, for each stateful component
\B{inst}, the argument is a reference to the aggregate representing the
state and named \llvm{\%argN}, where \llvm{N} is a number:
\begin{align*}
  \trad{}{args} \isdef & \llvm{""} \\
  \trad{\B{inst} L}{args} \isdef &
  \llvm{"\trad{\B{inst}, \textit{index}(\B{inst}, L)}{arg}, \trad{L}{args}"} \\
  \trad{\B{inst}, $n$}{arg} \isdef &
  \llvm{""} \triangleleft \stateless(\B{inst.mach}) \triangleright 
  \llvm{"\PH{\stateref(\B{inst.mach})} \%arg\PH{n}"}
\end{align*}
...
\begin{align*}
  ...
  & \quad \llvm{"store \PH{?ref} \%arg\PH{?X}, \PH{?ref} * \PH{\idx(\B{impo})}\nl}\\
