...
different functionalities provided by the implementation.\DD{Include the
possibility that the import is a machine without implementation. Instead it
has a list of typed, implementable, variables.}
  ...
  \begin{center}
    ...
    \begin{tabular}[t]{ccccc}
      \begin{tabular}[t]{rcl}
        ...
        \\
        \hline
        \multicolumn{3}{|c|}{Modules: \B{Module}} \\
        \hline
        \B{mach} & : & \B{MachRef} \\
        \B{pre} & : & opt \B{Name} 
        \\
        \hline
        \multicolumn{3}{|c|}{Machine: \B{MachRef}} \\
        \hline
        \B{spec} & : & \B{MachInterface} \\
        \B{impl} & : & opt \B{Impl} \\
        \hline
        \multicolumn{3}{|c|}{Machine interface: \B{MachInterface}} \\
        \hline
        \B{id} & : & \B{Name} \\
        \B{const} & : & seq \B{Cons} \\
        \B{var} & : & seq \B{Vari} \\
...
\item[basic] In that case its implementation is carried out outside the realm of
  the B method. A possible application is a set of related I/O routines. Even
  though no LLVM code needs to be generated to implement the basic machines, the
  code generator needs still to produce LLVM declarations of the entities
  (e.g., constants and operations) that are supplied by those machines.
...
illustrates this with the structure of a B project where an implementation
\B{I0} of a specification \B{M0} imports one unnamed instance of a basic machine
\B{M2} and two instances named \B{A} and \B{B} of a developed machine \B{M1}. In
turn its implementation \B{I1} itself imports two instances \B{C} and \B{D} of a
developed machine \B{M3}, implemented as \B{I3} (which has no imports itself).
The generation of LLVM code for the project shall allocate memory for one
instance of \B{I0}, two instances of \B{I2} and four instances of \B{I3}.
...
module being compiled we there be at run time. Consequently, the responsibility
for the static allocation of instances is assigned to the \emph{PROJ} mode,
which shall also perform the proper bindings between the actual and formal
instances in the generated code.
...
definition of a function named \llvm{@M\$op}The parameter list of the functions
thus produced contains one item for each input and output of the corresponding
operation. There is also one parameter of type \llvm{\%M\$ref\$}, to pass the
address of the structure encoding instance associated with that operation. Also
the translation produces a special function \llvm{@M\$init\$} responsible for
the initialization. The parameters of this function are the addresses of the
instances found in the import tree of the module (including the module itself).

Our approach consists in dividing the translation into three sections, and to
combine these sections differently in \emph{PROJ\/} and \emph{COMP\/} mode. For
a given module \B{M}, the sections are the following:
\begin{description}
\item[typedef:] Defines an LLVM data type \llvm{\%M\$state\$} corresponding to
  the data space of the module. In case the data space of a module is empty,
  this section is empty and the type is not defined:
  \begin{tabbing}
    \llvm{\%M\$state\$ = type \{ \ListOf{\nt{type}} \}}
  \end{tabbing}
\item[interface:] Defines an LLVM type \B{M\_ref}, pointer to \B{M\_data}, if
  this has been defined. 
  \begin{tabbing}
    \llvm{\%M\$ref\$ = type \%M\$state\$*}
  \end{tabbing}
  The interface also contains declarations of LLVM
  functions: one implementing the initialisation of the module, and one for each
  operation of the module. If the data space of the module is empty, no function
  definition corresponding to the initialisation is emitted
  \begin{tabbing}
    foo \= \kill
    \llvm{declare void @M\$op(\%M\$ref\$, \ListOf{\nt{type}})} \\
    \llvm{declare void @M\$init\$(\%M\$ref\$, \ListOf{\nt{type}})}
  \end{tabbing}
\item[implementation:] Defines the functions declared in the interface. This
  section is produced only for the developed modules.
  \begin{tabbing}
    foo \= \kill
  ...
  \end{tabbing}
\end{description}
Assuming these three sections are available, the \emph{COMP\/} translation
assembles them as follows:
\begin{center}
  \begin{tabbing}
    foo \= foo \= \kill
    \textit{for each imported module} \B{Q} \textit{generate} \\
    \> \textit{generate a type declaration for the state space of} \B{Q} \\
    \> \llvm{\%Q\$state\$ = type opaque} \\
