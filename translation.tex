...
%         \B{body} & : & \B{Inst} \\
  ...
  \end{center}
\end{figure}
\DD{Include \B{Vari} as one of the alternative for an \B{Expr}.}
...
\begin{align*}
  ...
  \trad{n.arg}{\B{expr}} \IN \LET w = \variable \IN \\
  & \llvm{"\PH{p} \quad \PH{w} = add i32 1, \PH{v} \nl"} \sep w \sep \llvm{i32}
\end{align*}
...
LLVM block with $\ell$ as exit point. It is defined as follows: \DD{Rewrite so
  that the temporary variable names are generated in the correct order.}

\begin{align*}
\trad{\B{il}, $\ell$}{\ListOf{\B{Inst}}}_L \isdef & \IF \B{il} \mbox{ is empty} \THEN \llvm{"br label \%\PH{\ell} \nl"} \\
& \ELSE \LET \B{i}, \B{il'} = \B{il}  \AND p = \trad{il', $\ell$}{\ListOf{\B{Inst}}}_L \IN \\
& \quad \IF \B{i} \mbox{ is \B{If} or \B{While}} \THEN \LET \ell' = \lbl \IN \llvm{"\PH{\trad{i, $\ell'$}{\B{Inst}}} \PH{\ell'}: \PH{p}"} \\
