...
\item interface: Defines an LLVM type \B{M\_ref}, pointer to \B{M\_data} (if
  ...
  \end{tabbing}
\item implementation: Defines the functions declared in the interface:
  \begin{tabbing}
    foo \= \kill
    ...
    \llvm{\}}
  \end{tabbing}
  This section is produced only for the developed modules.
\end{enumerate}
Based on the definition of these sections, the \emph{COMP\/} translation
...
generated LLVM artifacts, and using the existing LLVM tool set. Consequently,
the rules described in the preceding sections have been prototyped in the Python
programming language. The input of the translation is in a XML-based format,
named BXML, that is generated by the forthcoming version of Atelier-B. For each
B component (machine or implementation), Atelier-B generates a BXML file that
may then be used as input by our prototype. The output of the translation is in
LLVM-IR format. Such output may then be processed by the LLVM tools either to
generate binary in some compatible target platform or LLVM bitcode that may then
be executed using the LLVM interpreter. We used the latter approach to validate
our translation.
