...
\begin{align*}
  ...
  & \quad id = \Global(\B{c}) \AND ty = \state(\B{c.root}) \AND \\
  & \quad val = \IF \B{c.inst} = \text{None} \THEN \llvm{"\%self\$"} \ELSE \\
  & \quad \quad \llvm{"getelementptr \%self\$, i32 0, i32 \PH{\idx(\B{c.inst})}"}\\
  & \IN \\
  & \quad \llvm{"\PH{p_i}} \\
  & \quad \llvm{\PH{p_o}} \\
