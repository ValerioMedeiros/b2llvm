...
%TODO update the text showing that to get the address is needed to get a 
%sequence of steps.
...
type $t$. \VGM{Todo: update only the function LValue}
\begin{align*}
\begin{split}
  \trad{a}{\B{Beq}} \isdef
...
TODO: This function updates the Name function ?
TODO: Integrate with EXPArray
...
& \quad \IF \mbox{\B{n} is an array} \THEN \LET v = \variable \IN \trad{n}{AccessArray}  \sep v \sep t \\
...
%It can appear in LHS or RHS in become equal 
% We cannot use the tal function, because it is not able to solve any 
% expressions.
 - Não poderemos usar a função Tal, pois ela não resolverá expressões quaisquer!
\begin{align*}
\lefteqn{\trad{n}{AccessArray} \isdef} \\
&  \LET p_1 \sep v_1 \sep t_1  = \trad{base}{Exp}  \IN \\
&  \LET p_2 \sep v_2 \sep t_2  = \trad{indices}{Exp}  \IN \\
&  \LET v =  \variable  \IN \\
&  \llvm{"\PH{p_1} \PH{p_2} \PH{v} =  getelementptr \PH{t_1}, \PH{v_1}, i32 0, 
i32  \PH{v_2}\nl"} \sep v\\
&  \llvm{"\PH{v} = getelementptr \PH{\state(\B{n.root})}* \%self\$,\stateindex(\B{n.index,n.type})  \nl"}\\
\end{align*}
...
examples are presented in the reference manual of LLVM-IR. %TODO: put her a 
% reference of manual
...
This code stores a value in the record and get the value of element.
