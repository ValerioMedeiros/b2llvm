...
Two approaches are possibles to support arrays. An approach is importing
reusable libraries of AtelierB that are reported at official document of
...
more intermediate steps to manipulate the data of imported components. Not only,
this approach allows dynamic allocation for arrays, but it is extremely not
recommended to safety-critical systems. A second approach enjoys the direct 
correspondence between arrays native B0 and LLVM-IR and this approach is presented here.


A concrete array is a total function, where the domain indexes the range 
elements. The domain is composed by Cartesian product of sequence of concrete 
sets. The node Data array in AST can represent a declaration of array or a 
selection of element. The first element ``dec'' declares explicitly an array or does a 
reference to a variable array. The second element ``type'' defines a sequence of 
array types, where the last element defines the range of function and the others 
elements defines the domain. The third element ``indexes'' is optional, and it 
represents the selection of an element in array.
...
The B0 language supports attributions between arrays, even though the attribution
...
The arrays in B0 are represented by functions where an index pointer to an 
element. By default, the element of index not initialized has the zero value. 
...
The first parameter defines the type and name of variable, and the others are
...
integer values. When the array is multidimensional, news index must be added
